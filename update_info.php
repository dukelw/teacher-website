...
if (isset($_POST["name"]) || isset($_FILES["thumbnail"])) {
  ...
  $result = Teacher::update_information($_SESSION["useremail"], $thumbnail, $name, $email, $address, $phone, $gender, $birthday);
  if ($result) {
...
<form enctype="multipart/form-data" action="update_info.php" method="POST" class="container rounded bg-white mt-5 mb-5">
  <div class="row">
    <div class="col-md-3 border-right">
      <div style="padding-bottom: 12px !important;" class="d-flex flex-column align-items-center text-center py-5"><img class="user-avatar-big rounded-circle" src="<?= Teacher::get_teacher($_SESSION['useremail'])[0]["AVATAR"] ?>"> <span class="font-weight-bold"><?= Teacher::get_teacher($_SESSION['useremail'])[0]["NAME"] ?></span><span class="text-black-50"><?= Teacher::get_teacher($_SESSION['useremail'])[0]["MAIL"] ?></span><span> </span></div>
      <div class="col-md-12">
      ...
      <div style="visibility: hidden;" class="col-md-12 d-flex flex-column align-items-center thumbnail-wrapper">
        ...
        <img id="preview" class="user-avatar-big rounded-circle" src="<?= Teacher::get_teacher($_SESSION['useremail'])[0]["AVATAR"] ?>">
      </div>
    </div>
    <div class="col-md-9 border-right">
      <div class="p-3 py-5">
        ...
        <div class="row mt-3">
          <div class="col-md-12"><label class="labels">Tên</label><input disabled type="text" name="name" class="form-control" value="<?= Teacher::get_teacher($_SESSION['useremail'])[0]["NAME"] ?>"></div>
          <div class="col-md-12"><label class="labels">Email</label><input disabled type="text" name="email" class="form-control" value="<?= Teacher::get_teacher($_SESSION['useremail'])[0]["MAIL"] ?>"></div>
          <div class="col-md-12"><label class="labels">Số điện thoại</label><input disabled type="text" name="phone" class="form-control" value="<?= Teacher::get_teacher($_SESSION['useremail'])[0]["PHONE"] ?>"></div>
          <div class="col-md-12"><label class="labels">Địa chỉ</label><input disabled type="text" name="address" class="form-control" value="<?= Teacher::get_teacher($_SESSION['useremail'])[0]["ADDRESS"] ?>"></div>
          <div class="col-md-12">
            <label class="labels">Giới tính</label>
            <select name="gender" disabled type="text" class="form-control">
              <option value="_">Giới tính</option>
              <option value="1" <?php echo (Teacher::get_teacher($_SESSION['useremail'])[0]["GENDER"] == 1) ? 'selected' : ''; ?>>Nam</option>
              <option value="0" <?php echo (Teacher::get_teacher($_SESSION['useremail'])[0]["GENDER"] == 0) ? 'selected' : ''; ?>>Nữ</option>
            </select>
          </div>
        </div>
        <div class="row mt-3">
            ...
            <div class="row">
              <div class="col">
                <select disabled name="birthdayDay" class="form-control">
                  <?php
                  $selectedDay = date('d', strtotime(Teacher::get_teacher($_SESSION['useremail'])[0]["BIRTHDAY"]));
                  for ($day = 1; $day <= 31; $day++) {
                    echo '<option value="' . $day . '"';
                    if ($day == $selectedDay) echo ' selected';
                    echo '>' . $day . '</option>';
                  }
                  ?>
                </select>
              ...
              <div class="col">
                <select disabled name="birthdayMonth" class="form-control">
                  <?php
                  $selectedMonth = date('m', strtotime(Teacher::get_teacher($_SESSION['useremail'])[0]["BIRTHDAY"]));
                  $months = [
                    '01' => 'Tháng 1',
                    '02' => 'Tháng 2',
                    '03' => 'Tháng 3',
                  ...
                  foreach ($months as $key => $month) {
                    ...
                    if ($key == $selectedMonth) echo ' selected';
                    echo '>' . $month . '</option>';
                  }
                  ?>
                </select>
              ...
              <div class="col">
                <select disabled name="birthdayYear" class="form-control">
                  ...
                  $endYear = $currentYear;
                  $selectedYear = date('Y', strtotime(Teacher::get_teacher($_SESSION['useremail'])[0]["BIRTHDAY"]));
                  for ($year = $endYear; $year >= $startYear; $year--) {
                    echo '<option value="' . $year . '"';
                    if ($year == $selectedYear) echo ' selected';
                    echo '>' . $year . '</option>';
                  }
                  ?>
                </select>
            ...
            <div class="row">
              <div class="col">
                <select disabled name="joinDay" class="form-control">
                  <?php
                  $selectedDay = date('d', strtotime(Teacher::get_teacher($_SESSION['useremail'])[0]["JOINYEAR"]));
                  for ($day = 1; $day <= 31; $day++) {
                    echo '<option value="' . $day . '"';
                    if ($day == $selectedDay) echo ' selected';
                    echo '>' . $day . '</option>';
              ...
              <div class="col">
                <select disabled name="joinMonth" class="form-control">
                  <?php
                  $selectedMonth = date('m', strtotime(Teacher::get_teacher($_SESSION['useremail'])[0]["JOINYEAR"]));
                  $months = [
                    '01' => 'Tháng 1',
                    '02' => 'Tháng 2',
                    '03' => 'Tháng 3',
              ...
              <div class="col">
                <select disabled name="joinYear" class="form-control">
                  ...
                  $startYear = $currentYear - 100;
                  $endYear = $currentYear;
                  $selectedYear = date('Y', strtotime(Teacher::get_teacher($_SESSION['useremail'])[0]["JOINYEAR"]));
                  for ($year = $endYear; $year >= $startYear; $year--) {
                    echo '<option value="' . $year . '"';
                    if ($year == $selectedYear) echo ' selected';
                    echo '>' . $year . '</option>';
